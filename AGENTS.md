...
This repo is the canonical SOP. Claude, Cursor, Grok, Codex, and swarm
agents that start from `PLX_MC` follow the same rules. Consumer copies
(`AGENTS.md`, `CLAUDE.md`, `.cursorrules`, `.cursor/rules/mc-compliance.mdc`,
`prompts/uat-agent/v2/*`, `.github/copilot-instructions.md`, `GEMINI.md`)
must stay in sync — do not edit those consumer files from a PLX_MC PR.
...
repository slug match the intended work. Use
`scripts/compliance-checkout.mjs` when MCP metadata is missing or
mis-scoped — that is a live checkout, not a substitute stamp.
...
- Session-artifact capture runtime coverage: the Cursor `sessionEnd` hook (`.cursor/hooks.json`) and the Claude Code `SessionEnd` hook (`.claude/settings.json`) both run `scripts/compliance-closeout.mjs` and capture automatically; runtimes without hook support use the session-brain skill (see the agentic-swarm repo's `.cursor/skills/session-brain`) to submit manually
