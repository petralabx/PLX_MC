...
| AWS Secrets Manager (`prod/ec2-secrets`, us-east-1) | Active (local dev provisioning) | Owner: Vince. Scope: source of truth for the legacy shared MCP key and other runtime secrets; the `PLX_MC_MCP_AGENT_KEYS` field is a compatibility mirror only. Loaded on this box via `~/load-secrets.ps1` (per the "AWS Secrets Runtime" provisioning doc, 2026-06-10). Cloud Agents have no write access. |
| AWS Secrets Manager (`plx/prod/mc/mcp-agent-keys/v1`, us-east-1) | Active (MCP per-agent rotation) | Owner: Vince. Scope: source of truth for the serialized MCP per-agent key registry. Auth: `plx-mc-cloud-agent` may only Get, Describe, and Put this exact secret. Default: registry is synced to sensitive Vercel Production `PLX_MC_MCP_AGENT_KEYS` by `scripts/sync-mcp-agent-keys.py`. Kill switch: revoke inline policy `PLXMC-MCPAgentKeys-Rotate`; health: registry parse + production identity self-check; fallback: operator-managed compatibility mirror in `prod/ec2-secrets`; audit boundary: redacted hashes, IDs, statuses, and booleans only. |
...
| GitHub App (read-only repo Contents auth) | Active (App live; PAT fallback org-wide) | Owner: Vince. Scope: runtime (server-side read-only repo Contents/Metadata for loop-ledgers + repo validation; PR metadata + changed files for `mc_verify_pr` — needs Pull requests read on the App installation or the PAT fallback). Auth: GitHub App credentials from secrets manager — `GITHUB_APP_ID`, `GITHUB_APP_PRIVATE_KEY` (PEM), `GITHUB_APP_INSTALLATION_ID` / `GITHUB_APP_INSTALLATION_ID_PLX`; the module (`src/lib/github-app`) mints short-lived (≤1h) installation access tokens. When App mint is skipped/fails for **petralabx**, `resolveGithubToken` prefers `PETRALABX_GITHUB_TOKEN` (alias `PETRALABX_GITHUB`) over legacy `GITHUB_TOKEN`. Workstation hydrate: `. $HOME\load-secrets.ps1` and/or `. $HOME\.secrets-env.github.ps1` (see `docs/runbooks/petralabx-github-token-workstation.md`). Kill switch: unset the `GITHUB_APP_*` secrets (reverts to PAT/degraded). Health: installation-token mint success (warns + falls back on failure). Fallback: `PETRALABX_GITHUB_TOKEN` → `GITHUB_TOKEN` → null → loud degraded rows. Audit boundary: no token/key ever logged. Provisioning: `docs/runbooks/github-app-provisioning.md`. |
| PLX-MC MCP (task lifecycle + swarm compose) | Default-OFF (shipped 2026-06-24) | Owner: Vince. Scope: split — HTTPS cursor API (`/api/cursor/*`) and Streamable HTTP (`/api/cursor/mcp`) ship with the Vercel web app at `https://mc.plxcustomer.io`; **stdio client** (`tools/plx-mc-mcp/`) and **agentic swarm** (loopback `127.0.0.1:8900`) are **operator-local only** and are not part of the Vercel deploy. Auth: per-agent keys `PLX_MC_MCP_AGENT_KEYS` (JSON map principal id → key, sourced from `plx/prod/mc/mcp-agent-keys/v1`; legacy shared `PLX_MC_MCP_API_KEY` from `prod/ec2-secrets` behind `PLX_MC_MCP_SHARED_KEY_ENABLED`) + allowlisted `X-MC-Operator-Email`; swarm via `SWARM_KEY_CMD`. Default: OFF — `PLX_MC_MCP_ENABLED=0`. Kill switch: same flag or remove MCP server from config; `SWARM_DISPATCH_ENABLED=0` disables dispatch tools only. Health: `mc_self_check`. Read tools `mc_get_task` / `mc_list_checkouts` / `mc_search_knowledge` / `mc_verify_pr` need `task.read` only, write nothing (`mc_verify_pr` computes the gate verdict with `recorded: false`), and never return a usable `dsp_*` checkout id (redacted to `checkoutRef`, `dsp_…` + last 4 — a live id is a completion credential); `mc_request_approval` is the one wave-4 write (`approval.request` + restricted-project guard). Failed tools return `isError` + `{ error: { code, message, hint? } }`. Fallback: `scripts/mc.mjs` (operator-local REST CLI — `checkout` / `complete` / `status` against `/api/cursor/*`; inert until invoked, needs `MC_MCP_API_KEY` + `MC_OPERATOR_EMAIL`, same server kill switch; refuses `complete` without `--verify` + `--rollback`), `scripts/compliance-checkout.mjs`, `/api/compliance/*`, `bin/swarm ask`. Audit: `mcp.tool.invoked` in `mc_events`. Module: `docs/modules/mcp/README.md`. |
...
| VMC knowledge (Ask the Brain) | Active (Vercel env wired 2026-08-18, TASK-1075) | Owner: Vince. Scope: runtime — MC UI `/?screen=brain-ask`, `/api/brain-ask/*`, and MCP `mc_search_knowledge` (`/api/cursor/knowledge/search`) search VMC at `VMC_BASE_URL` (default `https://missioncontrol.tayloralton.com`). Auth: `VMC_API_KEY` via `vmcApiKey()` in `src/lib/secrets.ts` (source `prod/ec2-secrets`; Vercel Production/Preview). Default: `status: not_configured` empty hits when the key is missing; `upstream_unreachable` / `upstream_error` when VMC is down. Kill switch: unset `VMC_API_KEY` on Vercel. Health: `mc_self_check.brainAskConfigured` plus `brainAskSearchOk` / `brainAskSearchStatus` (no snippets). Fallback: MCP `plx-brain` / VMC `/vmc/second-brain`. Audit: no key logged. |
...
AWS Secrets Manager in `us-east-1` is authoritative. `prod/ec2-secrets`
contains the legacy shared MCP key and general runtime secrets.
`plx/prod/mc/mcp-agent-keys/v1` is the dedicated source of truth for the
