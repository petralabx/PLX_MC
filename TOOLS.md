...
| VMC knowledge (Ask the Brain) | Active (Vercel env wired 2026-08-18, TASK-1075) | Owner: Vince. Scope: runtime — MC UI `/?screen=brain-ask` and `/api/brain-ask/*` search VMC at `VMC_BASE_URL` (default `https://missioncontrol.tayloralton.com`). Auth: `VMC_API_KEY` via `vmcApiKey()` in `src/lib/secrets.ts` (source `prod/ec2-secrets`; Vercel Production/Preview). Default: fail-open empty hits when the key is missing. Kill switch: unset `VMC_API_KEY` on Vercel. Health: `mc_self_check.brainAskConfigured` (boolean only). Fallback: MCP `plx-brain` / VMC `/vmc/second-brain`. Audit: no key logged. |
