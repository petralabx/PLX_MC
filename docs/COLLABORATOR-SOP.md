...
machine. Do both steps below once per laptop (and again after the company allowlist
changes).
...
Instead, use the **company allowlist** in `config/company-skills-allowlist.json` and
the bootstrap scripts in this repo:
...
1. Clone or update `~/agentic-swarm` (or `%USERPROFILE%\agentic-swarm` on Windows).
2. Copy **only** allowlisted skills into `~/.cursor/skills` and `~/.claude/skills`.
3. Merge any **project-native** skills from this repo (e.g. `mc-sync`).
4. Mirror the combined set into the project’s `.cursor/skills/` and `.agents/skills/`.
...
To refresh after allowlist updates: re-run the same command.
...
ships, propose additions by PR to `config/company-skills-allowlist.json` and the
skill source in `agentic-swarm`.
