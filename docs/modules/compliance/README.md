...
- `src/lib/compliance/go-live-announcer.ts` — one-line Teams Workflow posts on `checkout` / `pr.opened` / `task.completed` (TASK-1454/1699; chat-primary, hard dedupe per task+event, kill switches default off; `ANNOUNCE_CHECKOUT` stays off unless operators re-enable)
