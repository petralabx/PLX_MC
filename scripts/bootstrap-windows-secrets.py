...
def github_powershell_reset_lines() -> list[str]:
    return [
        "# Keep git/gh on the authenticated gh keyring OAuth credential.",
        "Remove-Item Env:GITHUB_TOKEN -ErrorAction SilentlyContinue",
        "Remove-Item Env:GH_TOKEN -ErrorAction SilentlyContinue",
    ]


def github_powershell_loader_lines(token: str) -> list[str]:
    esc = token.replace("'", "''")
    return [
        "# GitHub org PAT for explicit petralabx API consumers — do not commit",
        ...
        *github_powershell_reset_lines(),
        "",
    ]


def github_shell_loader_lines(token: str) -> list[str]:
    return [
        "# GitHub org PAT for explicit petralabx API consumers — do not commit",
        "# Generated by scripts/bootstrap-windows-secrets.py",
        "# Source: AWS Secrets Manager prod/ec2-secrets (PETRALABX_GITHUB_TOKEN)",
        f"export PETRALABX_GITHUB_TOKEN={sh_quote(token)}",
        "# Keep git/gh on the authenticated gh keyring OAuth credential.",
...
def main() -> None:
    ...
    if values.get("PETRALABX_GITHUB_TOKEN"):
        lines.extend(["", *github_powershell_reset_lines()])
    ...
    gh_token = values.get("PETRALABX_GITHUB_TOKEN") or ""
    if gh_token:
        OUTPUT_GITHUB_PS1.write_text(
            "\n".join(github_powershell_loader_lines(gh_token)),
            encoding="utf-8",
        )
        OUTPUT_GITHUB_SH.write_text(
            "\n".join(github_shell_loader_lines(gh_token)),
            encoding="utf-8",
        )
        try:
